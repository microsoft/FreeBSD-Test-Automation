...
if ($isDeployed)
{
	...
	$tmp = RunLinuxCmd -username $user -password $password -ip $hs1VIP -port $hs1vm1sshport -command "pkg install -y  netperf " -runAsSudo
	$tmp = RunLinuxCmd -username $user -password $password -ip $dtapServerIp -port $dtapServerSshport -command "pkg install -y  netperf " -runAsSudo
	
	LogMsg "Test Machine : $hs1VIP : $hs1vm1sshport"
	LogMsg "DTAP Machine : $dtapServerIp : $hs1vm1sshport"
	$iperfTimeoutSeconds = $currentTestData.iperfTimeoutSeconds
	$cmd1="$python_cmd start-netperf-server.py"
	$cmd2="$python_cmd start-netperf-client.py -H $dtapServerIp -p $dtapServerUdpport -l $iperfTimeoutSeconds -u yes"
	...
	foreach ($Value in $SubtestValues) 
	{
		...
		{
			$testResult = $null
			$client.cmd = "$python_cmd start-netperf-client.py -H $dtapServerIp -p $dtapServerUdpport -l 10 -u yes -m $Value"
			...
			$testResult = NetperfClientServerUDPDatagramTest $server $client
