...
if($isDeployed)
{
	...
	{ 
		try
		{
			...
			{
				$client.cmd = "$python_cmd start-client.py -c $hs1vm1IP -i1 -p $hs1vm1tcpport -t$iperfTimeoutSeconds"
			}

			if(($mode -eq "URL") -or ($mode -eq "Hostname"))
			{
				$client.cmd = "$python_cmd start-client.py -c $hs1vm1Hostname -i1 -p $hs1vm1tcpport -t$iperfTimeoutSeconds"
			}
