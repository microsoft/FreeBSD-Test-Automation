	...
	foreach ($Value in $SubtestValues) 
	{
		...
		{   
			try
			{
				...
				{
					$client.cmd = "$python_cmd start-client.py -c $hs1VIP -i1 -p $hs1vm1udpport -t$iperfTimeoutSeconds -P$Value -u yes -l1420"
				}
				if(($mode -eq "URL") -or ($mode -eq "Hostname")){
					$client.cmd = "$python_cmd start-client.py -c $hs1ServiceUrl  -i1 -p $hs1vm1udpport -t$iperfTimeoutSeconds -P$Value -u yes -l1420"
				}
