	...
	foreach ($Value in $SubtestValues) 
	{
		...
		{	  
			try
			{
				...
				{
					$client.cmd = "$python_cmd start-client.py -c $hs1vm1IP  -p $hs1vm1udpport -t$iperfTimeoutSeconds  -P$Value -u yes"
				}

				if(($mode -eq "URL") -or ($mode -eq "Hostname"))
				{
					$client.cmd = "$python_cmd start-client.py -c $hs1vm1Hostname  -p $hs1vm1udpport -t$iperfTimeoutSeconds -P$Value -u yes"
				}
