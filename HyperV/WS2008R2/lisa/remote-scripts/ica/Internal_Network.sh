...
echo "########################################################"
echo "This is Test Case to test Internal Network"
...
sts=$?
             if [ 0 -ne ${sts} ]; then
				...
				dbgprint 1 "Cannot Proceed further with the test" 
             		...
             		dbgprint 1 "Aborting test."
             		UpdateTestState "TestAborted"
				UpdateSummary "Static IP for internal network is : Failed"      

             		exit 1
        		else
             		dbgprint 1  "Static IP: $static_ip set to $DEVICE : Success"  
			     UpdateSummary "Static IP for internal network is set to : $static_ip"      
        		fi
ifconfig $DEVICE up
...
sts=$?
        if [ ${sts} -ne "0" ]; then
		dbgprint 1  "Internal Network adapter card : $DEVICE  cannot ping the HOST SERVER INTERNAL IP :$HOST_SERVER_INTERNAL_IP"
		PASS=0
		UpdateSummary "Ping to Internal Network : Failed"                   
        else
		dbgprint 1  "Internal Network adapter card : $DEVICE  can ping the HOST SERVER INTERNAL IP :$HOST_SERVER_INTERNAL_IP  !!"
...
# if the return is Not Equal to 0 (successful)...
	...
	sts=$?
      if [ ${sts} -ne "0" ]; then
			dbgprint 1  "Internal Network adapter card : $DEVICE  cannot ping the REPOSITORY SERVER :$REPOSITORY_SERVER "
               UpdateSummary "Ping to External Network should fail : Success"      
        else
		    PASS=0
                dbgprint 1  "Internal Network adapter card : $DEVICE can ping REPOSITORY SERVER : $REPOSITORY_SERVER !!"
