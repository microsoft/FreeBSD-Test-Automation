...
# On FreeBSD the at job will not execute immediately even "now" is specified.
# Need to run "/usr/libexec/atrun" to kick off the job.
ssh -i /root/${TARGET_SSHKEY} root@${TARGET_IP} "echo '/root/iperf -s -D' | at now; /usr/libexec/atrun"
if [ $? -ne 0 ]; then
