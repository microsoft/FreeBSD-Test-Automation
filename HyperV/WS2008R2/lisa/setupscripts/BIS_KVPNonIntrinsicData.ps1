...
<#
.Synopsis
    FreeBSD VM creates a KVP item, then verify from the host.

.Description
    A FreeBSD VM will create a non-intrinsic KVP item.  Then
    ...
    to the following:
        <test>
            ...
            <testScript>FreeBSDScripts\KVP_Non_Intrinsic_Data.ps1</testScript>
...
.Example
    setupScripts\BIS_KVPNonIntrinsic.ps1 -vmName "myVm" -hvServer "localhost -TestParams "key=AAA;value=111;sshKey=rhel5_id_rsa.ppk"
...
$cmd = "./kvp_client -a -p 1 -k ${key} -v ${value}"
if (-not (SendCommandToVM $ipv4 $sshKey "${cmd}"))
{
