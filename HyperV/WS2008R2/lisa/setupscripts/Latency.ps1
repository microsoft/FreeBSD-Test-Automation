...
#
# While the IOZONE is running, ping the guest VM from a remote host and check the network latency
#
...
$connect = Invoke-Command -ComputerName ${RemoteServ} -ScriptBlock {param($ips) ping -n 100 $ips } -credential $cred -ArgumentList $ips
...
Write-Output "Average : ${avg}" | Out-File -Append $summaryLog
