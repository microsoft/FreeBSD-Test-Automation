############################################################################
# NMI_2.4.1-TriggerKernelPanic.ps1
#
# This script automates TC 2.4.1. It generates the test kernel
# panic on the VM, verifies if the vmcore files are created post kernel panic
# and if the vmcore files are readable using crash utility.
# 
...
Write-Output "Covers NMI TC-2.4.1" | Out-File $summaryLog
...
#
.\bin\pscp.exe -i ssh\${sshkey} ".\remote-scripts\bin\NMI_do_kernel_panic.sh" root@${ipv4}: 
if(!$?)
{
    "Error: Failed to copy NMI_do_kernel_panic.sh file"
    return $retVal
}
.\bin\plink -i ssh\${sshkey} root@${ipv4} "dos2unix /root/NMI_do_kernel_panic.sh"
.\bin\plink -i ssh\${sshkey} root@${ipv4} "chmod +x /root/NMI_do_kernel_panic.sh"
.\bin\plink -i ssh\${sshkey} root@${ipv4} "{ sleep 2; sh NMI_do_kernel_panic.sh; }"
