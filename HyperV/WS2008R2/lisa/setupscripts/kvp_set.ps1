...
if (-not $vmName -or $vmName.Length -eq 0)
{
  ...
  $DHCPEnabled = $false
  $IPAddress = "172.25.220.133"
  $Subnet = "255.255.255.0"
  $DnsServer = "10.171.64.23"
...
$TestName = "SetGuestNetworkAdapterConfiguration"
...
#Fill Up the IP address Inputs 
$nwconfig.DHCPEnabled = $DHCPEnabled
$nwconfig.IPAddresses = @($IPAddress)
$nwconfig.Subnets = @($subnet)
$nwconfig.DefaultGateways = @($Gateway)
...
write-output "SetGuestNetworkAdapterConfiguration"
$opresult = $vmservice.SetGuestNetworkAdapterConfiguration($vm.Path, @($nwconfig.GetText(1)))
MonitorJob($opresult)

write-output "Msvm_GuestNetworkAdapterConfiguration After update .. please make sure VM is running ..."
[System.Management.ManagementObject] $nwconfig = @(GetGuestNetworkAdapterConfiguration($VmName))[0];
write-output "-------------------------------------------------------------------------"
