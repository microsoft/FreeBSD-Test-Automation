...
{
	"Info : To import HyperV module from ${rootDir}\HyperVLibV2Sp1\Hyperv.psd1"
...
$serviceEnabled = Get-VMIntegrationComponent  -VM $vmName  -Server $hvServer | Where-Object {$_.description -like  "Microsoft Key-Value Pair*"}  | Where-Object {$_.enabledstate -eq [VMstate]::Stopped} 
if( $serviceEnabled )
{
...
#
$Vm = Get-WmiObject -Namespace root\virtualization -ComputerName $hvServer -Query "Select * From Msvm_ComputerSystem Where ElementName=`'$VMName`'"
if (-not $Vm)
{
...
$Kvp = Get-WmiObject -Namespace root\virtualization -ComputerName $hvServer -Query "Associators of {$Vm} Where AssocClass=Msvm_SystemDevice ResultClass=Msvm_KvpExchangeComponent"
if (-not $Kvp)
{
