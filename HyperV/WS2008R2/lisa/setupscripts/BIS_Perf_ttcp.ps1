...
#
if (-not (SendCommandToVM $ipv4 $sshKey "cc ./${ttcpFile} -o ./ttcp"))
{
    "Error: Unable to build ttcp.c on test VM"
    return $False
}

if (-not (SendCommandToVM $targetIP $sshKey "cc ./${ttcpFile} -o ./ttcp"))
{
    "Error: Unable to build ttcp.c on test VM"
    return $False
}
...
if (-not $sts)
{
	"Error: Failed to do SendCommandToVM on ${ipv4}"
	return $sts
}
