...
function GetVmSettingData([String] $name, [String] $server)
{
    ...
    $vssd = gwmi -n root\virtualization -class Msvm_VirtualSystemSettingData -ComputerName $server
    if (-not $vssd)
    {
        return $null
    }
...
function SetMMIOGap([INT] $newGapSize)
{
    ...
    #
    $mgmt = gwmi -n root\virtualization -class Msvm_VirtualSystemManagementService -ComputerName $hvServer
    if(-not $mgmt)
    {
        return $false
    }
