############################################################################
#
# ResizeAddVhdxDisk.ps1
#
# This script assumes it is running on a Server 2012 system with Hyper-V
# management installed.
#
# Description:
#     This is a LiSA setup script which runs before the VM is booted.
#     The script will create a .vhdx file, add the controller and
#     hard drive if necessary, then attach the .vhdx file to the
...
#     environment running LiSA.
#
#     The .xml entry to specify this startup script would be:
#
#         <setupScript>SetupScripts\AddVhdxDisk.ps1</setupScript>
#
#     LiSA always passes setup/cleanup scripts the vmName, hvServer, and
#     a string of testParams from the test definition.  This testParam
#     string is a semicolon separated string.  This script uses the following
#     testParams:
#         ControllerType
#         ControllerNumber
...
#         Sector Size
#
#     In the .xml file, the testParams have the format of:
#
#       ControllerType=ControllerNumber, ControllerLocation, Vhdx type, sector size
#
#     Some examples
#
...
#
#     The following is from an .xml file:
#
#       <testParams>
...
#       <testParams>
#
#     As mentioned earlier, these testParams will be parsed into the following string
#     by the LiSA scripts and passed to the setup/cleanup script:
#
                          ...
                          [int] $Lun, [string] $vhdType, [string] $sectorSize)
{
    ...
    else
    {
        ...
        #
        $vmInfo = Get-VM -Name $vmName -ComputerName $server
        if (-not $vmInfo)
        {
        ...
        $defaultVhdPath = $vmInfo.Path
        if (-not $defaultVhdPath.EndsWith("\"))
        {
            $defaultVhdPath += "\"
        }

        $vhdName = "${defaultVhdPath}${vmName}-${controllerType}-${controllerID}-${lun}-${vhdType}-${SectorSize}.vhdx" 
