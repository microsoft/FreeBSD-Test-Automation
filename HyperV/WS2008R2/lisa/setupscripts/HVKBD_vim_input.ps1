...
function getKeyboard($vmName)
{
    ...
    $cs = gwmi -computername "." -Namespace root\virtualization -class Msvm_computersystem -filter $filt
    $path = ${cs}.path.path
    $query2 = "ASSOCIATORS OF {$path} WHERE resultClass = Msvm_Keyboard"
    $Keyboard = gwmi -computerName "." -Namespace "root\virtualization" -Query $query2
    return $Keyboard
}
