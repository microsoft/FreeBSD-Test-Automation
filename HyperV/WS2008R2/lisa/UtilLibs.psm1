...
Function FinishLogTestSuite([object]$testsuite, [Float]$totalElapseTime)
{
	...
	($testsuite.testsuiteNode.Attributes | Where-Object { $_.Name -eq "time" }).Value = $totalElapseTime
...
Function FinishLogTestCase([object]$testcase, [string]$stateTimeStamp, [string]$result="PASS", [string]$message="", [string]$detail="")
{
	...
	$tmpValue = ($testcase.testsuite.testsuiteNode.Attributes | Where-Object { $_.Name -eq "tests" }).Value
	($testcase.testsuite.testsuiteNode.Attributes | Where-Object { $_.Name -eq "tests" }).Value = [int]$tmpValue + 1
	if ($result -eq "FAIL")
	{
        ...
        $tmpValue = ($testcase.testsuite.testsuiteNode.Attributes | Where-Object { $_.Name -eq "failures" }).Value
        ($testcase.testsuite.testsuiteNode.Attributes | Where-Object { $_.Name -eq "failures" }).Value = [int]$tmpValue + 1
	}
	...
	if ($result -eq "ERROR")
	{
        ...
        $tmpValue = ($testcase.testsuite.testsuiteNode.Attributes | Where-Object { $_.Name -eq "errors" }).Value
        ($testcase.testsuite.testsuiteNode.Attributes | Where-Object { $_.Name -eq "errors" }).Value = [int]$tmpValue + 1
	}
	FinishLogReport $False
}
...
Function CIWaitForZipOperationToFinish([__ComObject] $zipFile, [int] $expectedNumberOfItemsInZipFile, [int] $timeout)
{
    ...
    {
        Throw "Time-out exceeded waiting for zip operation"
    }
    ...
    {
        Write-Debug "The zip operation completed successfully."
    }
    ElseIf ($count -eq 0)
    {
    ...
    {
		# "Zip file: $count  and expected number of items: $expectedCount"
        # Throw "Zip file contains more than the expected number of items."
        Throw "Zip file contains more than the expected number of items. Zip file: $count  and expected number of items: $expectedCount"
    }
	#>
}
