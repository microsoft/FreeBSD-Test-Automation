...
#
#         <setupScript>SetupScripts\AddVhdxDiffDisk.ps1</setupScript>
#
...
function GetPhysicalDiskForPassThru([string] $server)
{
    ...
    foreach ($vm in $VMs)
    {
        ...
        #$VMSettingData = Get-WmiObject -Namespace "root\virtualization\v2" -Query $query -ComputerName $server
        #
        #if ($VMSettingData)
        #{
            # 
        ...
        #    $PhysicalDiskResource = Get-WmiObject -Namespace "root\virtualization\v2" -Query $query `
                          ...
                          [int] $Lun, [string] $vhdType, [string] $sectorSizes)
{
    ...
    else
    {
        ...
        if (-not $fileInfo)
        {
            ...
            $nv = New-Vhd -ParentPath $parentVhdName -Path $vhdName -ComputerName $server -Differencing
            if ($nv -eq $null)
            {
