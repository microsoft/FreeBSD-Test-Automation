...
<#
.Synopsis
    Delete a non-existent KVP item from a FreeBSD guest.
.Description
    Delete a non-existent KVP item from pool 0 on a FreeBSD guest.
...
.Example
    setupScripts\BIS_DeleteKeyValueFail.ps1 -vmName "myVm" -hvServer "localhost -TestParams "key=aaa;value=222"
