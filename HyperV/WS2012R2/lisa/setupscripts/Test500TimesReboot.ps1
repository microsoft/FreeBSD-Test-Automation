############################################################################
#
# Test500TimesReboot.ps1
#
# Description:
#     This is a PowerShell test case script that runs on the on
#     the ICA host rather than the VM.
#
#     This script will reboot a VM 500 times and check that the VM reboots successfully.  
#     
#
...
Write-Output "Covers BOOT-02" | Out-File $summaryLog
...
#
# Set the count and reboot the machine 500 times. 
#

$count = 500
$bootcount = 0

Write-Output "setting the count to 500 for rebooting the VM" | Out-File  ${rootDir}\Test500TimesReboot.log
...
while ($count -gt 0)
{
  ...
  Start-Sleep -seconds 10
   ...
   $count -= 1
   $bootcount += 1
   Write-Output "Boot count:"$bootcount | Out-File -Append ${rootDir}\Test500TimesReboot.log
}

#
# If we got here, the VM was rebooted 500 times successfully 
#
$retVal = $true

Write-Output "VM rebooted 500 times successfully" | Out-File -Append $summaryLog
