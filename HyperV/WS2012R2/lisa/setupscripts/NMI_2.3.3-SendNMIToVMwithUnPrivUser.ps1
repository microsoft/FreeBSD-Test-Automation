...
#$newJob = Invoke-Command -ComputerName localhost -ScriptBlock $cmd -Credential $creds -AsJob 
...
While ($job.State -ne "Completed")
{
    ...
    {
        Write-Output "Job Failed" | Out-File -Append $summaryLog
        return $false
    }
    start-sleep 2
}
...
$nmistatus = Receive-Job -Id $newJob.Id -Wait -WriteJobInResults -WriteEvents
