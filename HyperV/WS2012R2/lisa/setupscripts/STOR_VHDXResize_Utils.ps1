...
{
    "#!/bin/bash`nexec ./${filename}.sh >& ${filename}.log" | out-file -encoding ASCII -filepath runtest.sh
    ...
    if (-not $?)
    {
     ...
     .\bin\pscp.exe -i ssh\${sshKey} .\remote-scripts\freebsd\${filename}.sh root@${ipv4}:
    if (-not $?)
    {
       Write-Error -Message "Error: Unable to copy ${filename}.sh to the VM" -ErrorAction SilentlyContinue
       return $False
    ...
    .\bin\plink.exe -i ssh\${sshKey} root@${ipv4} "dos2unix ${filename}.sh  >& /dev/null"
    if (-not $?)
    {
    ...
    .\bin\plink.exe -i ssh\${sshKey} root@${ipv4} "chmod +x ${filename}.sh   >& /dev/null"
    if (-not $?)
    {
