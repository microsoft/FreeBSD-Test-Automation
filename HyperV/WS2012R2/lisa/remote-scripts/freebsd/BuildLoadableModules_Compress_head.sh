...
rm -rf ~/freebsd-head-current.tar.bz2
...
if [ ! -e /mnt/archives/freebsd-head-current.tar.bz2 ]; then
    LogMsg "Error: freebsd-head-current.tar.bz2 file does not exist"
    UpdateTestState $ICA_TESTFAILED
    echo "Cloned GitHub    : Failed :: freebsd-head-current.tar.bz2 file not found" >> ~/summary.log
    exit 50
fi
...
LogMsg "cp ./freebsd-head-current.tar.bz2 /usr/"
cp ./freebsd-head-current.tar.bz2 /usr/
if [ $? -ne 0 ]; then
    LogMsg "Error: Unable to copy freebsd-head-current.tar.bz2 from repository server export"
    UpdateTestState $ICA_TESTFAILED
    echo "Cloned GitHub    : Failed :: Unable to copy freebsd-head-current.tar.bz2 file" >> ~/summary.log
    exit 60
fi
cp ./GENERIC /usr/
if [ $? -ne 0 ]; then
    LogMsg "Error: Unable to copy GENERIC from repository server export"
    UpdateTestState $ICA_TESTFAILED
    echo "Cloned GitHub    : Failed :: Unable to copy GENERIC file" >> ~/summary.log
    exit 60
fi
...
#
LogMsg "Info : tar -xjf ./freebsd-head-current.tar.bz2"

if [ ! -e ./freebsd-head-current.tar.bz2 ]; then
    LogMsg "Error: the freebsd-head-current.tar.bz2 file was not copied"
    UpdateTestState $ICA_TESTFAILED
    exit 65
fi

tar -xjf ./freebsd-head-current.tar.bz2
if [ $? -ne 0 ]; then
    LogMsg "Error: Unable to extract files from freebsd-head-current.tar.bz2"
    UpdateTestState $ICA_TESTFAILED
    exit 70
fi

if [ ! -e ./freebsd-head ]; then
    LogMsg "Error: The git clone did not create the directory /usr/freebsd-head"
    echo "Clone GitHub     : Failed :: /usr/freebsd-head directory not found" >> ~/summary.log
    UpdateTestState $ICA_TESTFAILED
    exit 80
fi
...
# Copy GENERIC File
#
cp /usr/GENERIC /usr/freebsd-head/sys/amd64/conf/
if [ $? -ne 0 ]; then
    LogMsg "Error: Unable to copy GENERIC to conf"
    UpdateTestState $ICA_TESTFAILED
    echo "Cloned GitHub    : Failed :: Unable to copy GENERIC to conf" >> ~/summary.log
    exit 60
fi
...
if [ $? != 0 ]; then
    echo "Edit make.conf : Failed" >> ~/summary.log
    UpdateTestState $ICA_TESTFAILED
    exit 60
fi
