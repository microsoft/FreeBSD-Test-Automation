...
#Checking filesystem
#
fsck -y -v -f /dev/da1p1 >> ~/summary.log 2>&1
if [ $? -gt 0 ]; then
...
#
dd if=/dev/da1 of=/dev/da1 count=0 >> ~/summary.log 2>&1
gpart recover da1 >> ~/summary.log 2>&1
gpart resize -i 1 da1 >> ~/summary.log 2>&1
growfs -y /dev/da1p1 >> ~/summary.log 2>&1
if [ $? -gt 0 ]; then
