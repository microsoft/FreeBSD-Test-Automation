...
########################################################################
#
# FreeBSD on Hyper-V and Azure Test Code, ver. 1.0.0
...
#	inside a FreeBSD VM are processing VMBus interrupts, by checking 
...
#    2. Looks for the Hyper-v timer property of each CPU
...
#
cpu_count=`sysctl -a | grep -i hw.ncpu | cut -d ' ' -f 2`
...
#Write the interrupts info into /proc/interrupts
vmstat -i > /proc/interrupts
...
do
    if [[ $line = *hyperv* ]]; then
		...
		do
			cpux=`echo $line | grep -i cpu$i`
            if [ $? -eq 0 ]; then
				intrCount=`echo $line | cut -f 2 -d ' '` 
