...
gpart list
sleep 2
NEW_DISK=$(dmesg | egrep '^da[0-9][^:]' | wc -l)
NEW_DISK=$((NEW_DISK-1))
...
# Format and mount the disk
		...
		if [ $? -eq 0 ]; then
				LogMsg "Deleting existing filesystem"
    ...
    sleep 2
		...
		newfs ${!j}p1
		if [ $? -ne 0 ]; then
				LogMsg "Error: Unable to format device ${TEST_DEVICE}p1"
				UpdateTestState $ICA_TESTFAILED
				exit 60
		fi
...
sts=$?
    if [ 0 -ne ${sts} ]; then
        echo "Error: make freebsd ${sts}" >> ~/summary.log
        UpdateTestState "TestAborted"
        echo "make freebsd: Failed" 
        exit 50
    else
        echo "make freebsd: Success"
