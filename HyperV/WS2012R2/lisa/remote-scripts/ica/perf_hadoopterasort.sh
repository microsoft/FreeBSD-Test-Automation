...
ConfigFreeBSD()
{
	LogMsg "ConfigFreeBSD"

	#
	# Install Java
	#
	# On FreeBSD we'll use OpenJDK (java/openjdk17)
	#
	LogMsg "Check if Java is installed"
	javaInstalled=`which java`
	if [ ! $javaInstalled ]; then
		LogMsg "Installing Java"
		pkg install -yf java/openjdk7
		if [ $? -ne 0 ]; then
			LogMsg "Error: Unable to install java"
			UpdateTestState $ICA_TESTFAILED
			exit 1
	...
	JAVA_HOME="/usr/local/openjdk7/jre"

	#
	# Extraly, we need to mount /dev/fd and /proc
	#
...
# fix permissions on the id_rsa keys
...
sed -i "~" -e "s~export JAVA_HOME=\${JAVA_HOME}~export JAVA_HOME=${JAVA_HOME}~g" ./hadoop-env.sh
if [ $? -ne 0 ]; then
...
sed -i "~" -e "s~</configuration>~    <property>~g" ./core-site.xml
...
sed -i "~" -e "s~</configuration>~    <property>~g" ./yarn-site.xml
...
sed -i "~" -e "s~</configuration>~    <property>~g" ./mapred-site.xml
...
sed -i "~" -e "s~</configuration>~    <property>~g" ./hdfs-site.xml
