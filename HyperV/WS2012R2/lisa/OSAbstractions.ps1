...
#
#####################################################################
function SubmitAtJob ([System.Xml.XmlElement] $vm)
{
	<#
	.Synopsis
		Submits the runtest.sh job to at daemon

    .Description
    	Schedules the job to run by calling "at -f runtest.sh now"
	#> 
    #$os = bin\plink -i ssh\${sshKey} root@${hostname} "uname -s"
		...
		if ($vm.os)
		{
			switch ($($vm.os))
			{
				$LinuxOS   
				{
					if ( (SendCommandToVM $vm "at -f runtest.sh now") )
					{
						$jobSubmitted = $True
					}
				}
				$FreeBSDOS 
				{
					if ( (SendCommandToVM $vm "at -f runtest.sh now") )
					{
# On FreeBSD, jobs scheduled at "now" will NOT execute
# immediately but will be executed *every 5 minutes*
# by crond.
# 
# To get it run immediately, we must call /usr/libexec/atrun
						if ( (SendCommandToVM $vm "/usr/libexec/atrun") )
						{
							$jobSubmitted = $True
						}
					}
