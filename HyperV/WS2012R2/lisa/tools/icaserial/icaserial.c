...
HandleReadPipeLoop(IN LPWSTR wszPipeName)
{
	...
	while (TRUE)
	{
		...
		while (TRUE)
		{
			...
			fprintf(f, "%s", pConnection->pRequest); //This is for copy the logs to file
			fflush(f);
			pConnection->dwRequest = 0;
		}
...
// This is for creating the file name string based on current date and time
void set_file_name_string(void )
{
	struct tm tim;
	time_t now;
	now = time(NULL);
	tim = *(localtime(&now));
	strftime(file_name,30,"%b-%d-%Y-%H-%M-%S",&tim);
